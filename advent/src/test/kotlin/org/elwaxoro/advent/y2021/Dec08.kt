...
class Dec08 : PuzzleDayTester(8, 2021) {
     ...
     */
    override fun puzzle1(): Any = load().map { it.split(" | ").let { it[0] to it[1] } }.sumOf { it.second.split(" ").filter { it.length in listOf(2, 3, 4, 7) }.size }
     ...
     */
    override fun puzzle2(): Any = load().map { it.replace(" |", "").split(" ") }.map { it to it.buildMasks() }.map { (digits, masks) ->
        digits.map { digit ->
            when(digit.length) {
                2 -> 1
                3 -> 7
